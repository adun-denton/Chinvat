...
**هشدار صریح:** `theme-write` عمداً امکان Remote Code Execution می‌دهد. فقط از Application password مخصوص Admin استفاده کنید؛ MCP را در اختیار Caller نامطمئن قرار ندهید؛ Content نامطمئن را به Agent دارای Write access ندهید. PHP ورودی Agent ابتدا با Zend parser داخل همان Runtime بررسی می‌شود؛ اگر Tokenizer موجود نباشد، PHP CLI معتبر از مسیر `proc_open` نقش Fallback را دارد. اگر هیچ Backend معتبری موجود نباشد Write همچنان Fail closed رد می‌شود. Write فایل‌های غیر-PHP تحت تأثیر نیست. `functions.php` تولیدشده توسط Scaffold ورودی Agent نیست و از این مسیر عبور نمی‌کند. Path/Symlink confinement، Atomic rename، Backup محافظت‌شده و Option denylist ریسک را کاهش می‌دهند، اما امنیت مطلق ایجاد نمی‌کنند.
