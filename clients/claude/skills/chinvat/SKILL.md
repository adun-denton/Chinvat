...
1. **Discover** — call `workers_list` to see modules, their health, policy tier, and operations. Call `capabilities_describe {module}` for the exact operations, argument schemas, and each operation's risk (`read` / `act` / `dangerous`).
2. **Delegate** — call `tasks_submit {module, operation, args, mode}`.
   ...
   - `mode: "async"` returns a `job_id` immediately; poll `tasks_status` / `tasks_result`.
...
- **Offload bulk work to local models:** `tasks_submit {module:"ollama", operation:"chat", args:{model:"qwen3", messages:[...]}}` — keep large, cheap, or private work off the frontier model.
- **Reach a specialist:** `tasks_submit {module:"openrouter", operation:"chat", args:{model:"...", messages:[...]}}`.
- **Act on the machine:** `tasks_submit {module:"system", operation:"run_command", args:{command:"..."}}` — this is `dangerous` and will usually need approval.
