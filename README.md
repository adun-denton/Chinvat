...
The built-in `wordpress` module uses core WordPress REST for posts, pages, media, and taxonomy. The optional [Chinvat WP Bridge](wp-plugin/chinvat-bridge/README.md) 0.3.1 adds ten WordPress Abilities for guarded option access, active-theme file I/O, per-post RankMath fields, activating or deactivating installed plugins, and block-aware child-theme scaffolding. The scaffold now creates `style.css`, minimal `theme.json`, a trusted plugin-authored `functions.php` that loads the child stylesheet, copied header/footer parts when present, and `templates/`. Its authenticated handshake is `GET /wp-json/chinvat-bridge/v1/info`.
